...
namespace Runtime.Systems
{
    ...
    public sealed class AttackSystem : ISystem
    {
        ...
        public void OnAwake()
        {
            _unitFilter = World.Filter.With<NavMeshAgentComponent>().With<AttackComponent>().With<TargetComponent>().Without<IsNewTargetMarker>().Build();
        ...
        public void OnUpdate(float deltaTime)
        {
            foreach (var unitEntity in _unitFilter)
            {
                ...
                if (agentComponent.NavMeshAgent.remainingDistance < agentComponent.NavMeshAgent.stoppingDistance && targetComponent.TargetEntity != default)
                {
                    ...
                    {
                        bool isDisposed = this.World.IsDisposed(targetComponent.TargetEntity);
                        if (!isDisposed)
                        {
